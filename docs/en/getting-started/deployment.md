...
| Platform | CI runner | Artifacts | glibc floor |
|----------|-----------|-----------|-------------|
| macOS | `macos-latest` | `.dmg` | — |
| Linux | `ubuntu-22.04` | desktop `.deb` / `.AppImage` | 2.34 |
| Linux (server) | `ubuntu:20.04` container | `dinotty-server_*.deb` | 2.31 |
| Windows | `windows-latest` | NSIS `.exe`, portable `.exe` | — |

The server deb gets its own 20.04 container build because `$auto` derives the `libc6` floor from the build machine's glibc: the desktop must stay on 22.04 (WebKitGTK runtime libs), while the server links no GUI libraries at all (the frontend is embedded into the binary via `rust-embed`), so building it on focal lowers the floor to 2.31 and covers Ubuntu 20.04 / Debian 11. The workflow asserts the floor is exactly `2.31` so a silent runner upgrade cannot regress compatibility unnoticed.
